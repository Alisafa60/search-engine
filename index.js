...
class SearchEngine{
    constructor(documentCollection){
        ...
        this.stemmer = natural.PorterStemmer //stemming for finding root word
    }
    ...
    search(querry){
        ...
        (async() =>{
            const expandedQuerryTokens = await this.querrySynonyms(querryTokens);
            this.documentCollection.documents.forEach(document => {
                const distances = [];
                const wordMatches = [];
                const documentTokens = this.stemTokens(document.content); 
                const stemmedQuerryTokens = this.stemTokens(expandedQuerryTokens);
